...
def store_data(team_id, output_folder, start_gw):
    ...
    num_gws = len(summary["current"]) + 1 - start_gw
...
def get_team_data(team_id, season, start_gw):

    output_folder = "scripts/teams/team_" + team_id + "_data" + season
    # if not os.path.exists(output_folder):
    #     os.makedirs(output_folder)
    # store_data(team_id, output_folder, start_gw)

    df = pd.read_csv(output_folder + '/picks_' + str(start_gw) + '.csv')
    ...
    merged = merged.merge(df, left_on='id', right_on='element')
